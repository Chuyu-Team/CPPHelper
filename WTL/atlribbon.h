...
// Implementation
	                         ...
	                         const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* /*ppropvarNewValue*/)
	{
		...
		case k_ItemsSource:
			{
				ATL::CComPtr<IUICollection> pIUICollection(ppropvarCurrentValue->punkVal);
		...
		case k_Categories:
			if (t_categories != 0)
			{
				ATL::CComPtr<IUICollection> pIUICategory(ppropvarCurrentValue->punkVal);
...
// Implementation
		...
		switch (verb)
		{
			...
			case UI_EXECUTIONVERB_EXECUTE:
				...
				if (SUCCEEDED(hr = pCommandExecutionProperties->GetValue(UI_PKEY_FontProperties_ChangedProperties, &propvar)))
					m_cf << ATL::CComPtr<IPropertyStore>(propvar.punkVal);
				break;
	                                 ...
	                                 const PROPVARIANT* ppropvarCurrentValue, PROPVARIANT* ppropvarNewValue)
	{
		...
		{
			ATL::CComPtr<IPropertyStore> pStore(ppropvarCurrentValue->punkVal);
			m_cf >> pStore;
...
// QAT dock states
	UI_CONTROLDOCK GetQATDock()
	{
		...
		UINT32 uDock = 0;
		PROPVARIANT propvar;
		ATL::CComPtr<IPropertyStore>pIPS(GetRibbonPtr());
...
// Ribbon colors
	UI_HSBCOLOR GetRibbonColor(REFPROPERTYKEY key)
	{
		...
		PROPVARIANT propvar;
		ATL::CComPtr<IPropertyStore>pIPS(GetIUIFrameworkPtr());
